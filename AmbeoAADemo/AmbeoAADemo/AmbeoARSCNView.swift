...
class AmbeoARSCNView: ARSCNView {

    // MARK: Position Testing

    /** Hit test against the `sceneView` to find an object at the provided point */
    func findObject(at point: CGPoint) -> VirtualObject? {
    ...
    /** Hit test against the `sceneView` to find a plane at the provided point */
    func findPlane(at point: CGPoint) -> SCNNode? {
        let hitTestOptions: [SCNHitTestOption: Any] = [.boundingBoxOnly: true]
        let hitTestResults: [SCNHitTestResult] = hitTest(point, options: hitTestOptions)
        return hitTestResults.lazy.flatMap { result in
            return AmbeoARSCNView.existingPlaneNodeContainingNode(result.node)
            }.first
    }

    /** Generic "smart" hit test to find an existing or estimated plane */
    ...
    /** Returns a "planeNode" if one exists as an ancestor of the given node */
    static func existingPlaneNodeContainingNode(_ node: SCNNode) -> SCNNode? {
        if node.name == "planeNode" { return node }
        guard let parent = node.parent else { return nil }
        // Recurse through parents
        return existingPlaneNodeContainingNode(parent)
    }
