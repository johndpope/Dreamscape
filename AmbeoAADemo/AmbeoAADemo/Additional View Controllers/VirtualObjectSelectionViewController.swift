...
class ObjectCell: UITableViewCell {
    ...
    var objectName = "" {
        didSet {
            objectTitleLabel.text = objectName.capitalized
        }
    }

    func setObjectCellImage(named: String) {
        objectImageView.image = UIImage(named: named)
    }
...
class VirtualObjectSelectionViewController: UITableViewController {
    ...
    func updateObjectAvailability(for planeAnchor: ARPlaneAnchor?) {
        ...
        for (row, object) in VirtualObjectLoader.availableObjects.enumerated() {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.objectName = virtualObjects[indexPath.row].config.displayName
        cell.setObjectCellImage(named: virtualObjects[indexPath.row].config.modelName)
